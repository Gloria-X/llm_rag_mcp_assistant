...
export async function processTextDocument(filePath: string): Promise<void> {
  try {
    const processor = new DocumentProcessor();
    const result = await processor.processDocument(filePath);
    
    log('PROCESS TEXT DOCUMENT SUCCESS')
    console.log(`documentId: ${result.documentId}`, `segmentCount: ${result.segmentCount}`, `totalWords: ${result.totalWords}`)
  } catch (error) {
    console.error('processTextDocument err:', error);
    throw error;
  }
}
